...
type (
	...
	// Def represents the definition of an identifier (*ast.Ident in Field.Names, LabeledStmt.Label,
	// ImportSpec.Name, ValueSpec.Names, TypeSpec.Name, FuncDecl.Name, File.Name). Def does not
	...
	// Ref represents the reference of an identifier (*ast.Ident or *ast.SelectorExpr). Ref
	// implements the Expr interface.
	Ref struct {
		Name string
...
// ----------------------------------------------------------------------------
// Convenience functions for Def/Ref

// NewDef creates a new Def.
func NewDef(name string) *Def { return &Def{name, nil, DefDecorations{}} }

// NewRef creates a new Ref. Use an empty string for path for the local package.
func NewRef(name, path string) *Ref { return &Ref{name, path, nil, RefDecorations{}} }
...
//
func (id *Def) IsExported() bool { return IsExported(id.Name) }
func (id *Ref) IsExported() bool { return IsExported(id.Name) }

func (id *Def) String() string {
	if id != nil {
		return id.Name
	}
	return "<nil>"
