...
func parse(filename string, src interface{}) (*dst.File, error) {
	if *verbose {
...
func parseStdin() (*dst.File, error) {
...
func parseFiles(dir string, filenames []string) ([]*dst.File, error) {
	files := make([]*dst.File, len(filenames))
...
func parseDir(dir string) ([]*dst.File, error) {
	ctxt := build.Default
...
func getPkgFiles(args []string) ([]*dst.File, error) {
	if len(args) == 0 {
		// stdin
		file, err := parseStdin()
		if err != nil {
...
func checkPkgFiles(files []*dst.File) {
	type bailout struct{}
